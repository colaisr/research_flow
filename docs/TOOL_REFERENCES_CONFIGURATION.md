...
**Сейчас реализовано:**
- ✅ `natural_language` - автоматическое извлечение параметров из текста промпта
- ✅ Поддержка `query_template` для Database tools
- ✅ Поддержка `context_window` для всех типов инструментов

**Не реализовано:**
- ❌ `explicit` метод - явное указание параметров в синтаксисе
- ❌ `template` метод - использование шаблонов с переменными
- ❌ UI для выбора extraction method
...
### 1. Natural Language (Естественный язык) ✅ **РЕАЛИЗОВАНО**

**Как работает:**
Система анализирует текст вокруг ссылки на инструмент (например, `{binance_api}`) и автоматически извлекает параметры из контекста.

**⚠️ ВАЖНО: Реализовано через КОД (regular expressions), а НЕ через AI/LLM**

Текущая реализация использует:
- **Regular expressions** для поиска паттернов (BTC/USDT, H1, SELECT, etc.)
- **Pattern matching** для извлечения параметров
- **Правила** для нормализации форматов (1h → H1)

**Будущее улучшение:** Для RAG tools может использоваться LLM для более точного извлечения сложных запросов (см. раздел "Advanced RAG Extraction").
...
### 3. Template (Шаблон) ⚠️ **ЧАСТИЧНО РЕАЛИЗОВАНО**
